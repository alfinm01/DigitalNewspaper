...
def index():
	try:
		req_weather = requests.get('https://api.openweathermap.org/data/2.5/weather?q=' + 'Jakarta' + '&units=metric&appid=' + env.WEATHER_TOKEN)
		req_news = requests.get('https://newsapi.org/v2/top-headlines?country=' + 'id' + '&apiKey=' + env.NEWS_TOKEN)
...
def search():
	try:
		...
		req_weather = requests.get('https://api.openweathermap.org/data/2.5/weather?q=' + keyword + '&units=metric&appid=' + env.WEATHER_TOKEN)
		req_news = requests.get('https://newsapi.org/v2/everything?q=' + keyword + '&apiKey=' + env.NEWS_TOKEN)
...
def weather(region):
	try:
		weather = requests.get('https://api.openweathermap.org/data/2.5/weather?q=' + region + '&units=metric&appid=' + env.WEATHER_TOKEN)
...
def news(region):
	try:
		news = requests.get('https://newsapi.org/v2/top-headlines?country=' + region + '&apiKey=' + env.NEWS_TOKEN)
...
if __name__ == '__main__':
	app.run(host = '0.0.0.0', threaded = True, port = env.PORT)
